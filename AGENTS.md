...
- `src/lx_skills/` houses the core package (agent, CLI, tools, skill loader, and `stream/` formatting/tracking helpers).
...
- `pyproject.toml` defines dependencies and the `lx-skills` CLI entry point; `uv.lock` pins versions.

## Build, Test, and Development Commands
- `uv sync`: install dependencies into the local environment.
- `uv run lx-skills --interactive`: run the interactive CLI demo.
- `uv run lx-skills "列出当前目录"`: run a single prompt.
- `uv run lx-skills --list-skills`: verify Skills discovery.
...
- Keep CLI output and stream formatting behavior consistent with existing utilities in `src/lx_skills/stream/`.
